...
public class MyAuthenticationProvider implements AuthenticationProvider {
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		if (!userInfo.isEnabled()) {
			throw new BadCredentialsException("用户暂时不可用");
		}
		if (!userInfo.isCredentialsNonExpired()) {
			throw new BadCredentialsException("用户密码已过期");
		}
		/*
		 * if (!userInfo.getPassword().equals(password)) { //简单判断
		 * //System.out.println(password+userInfo.getPassword()); throw new
		 * BadCredentialsException("密码不正确"); }
		 */

		String enpasswd = MD5Util.encode(password);
		if (!userInfo.getPassword().equals(enpasswd)) {
			//System.out.println(enpasswd + userInfo.getPassword());
			throw new BadCredentialsException("密码不正确");
		}
